...
public class HoverAroundLightGoal extends Goal {
  ...
  public boolean canUse() {
    ...
    this.scanCooldown =
        SCAN_COOLDOWN_MIN + this.mob.getRandom().nextInt(SCAN_COOLDOWN_MAX - SCAN_COOLDOWN_MIN + 1);
  ...
  public void start() {
    this.orbitTicksRemaining =
        ORBIT_DURATION_MIN_TICKS
            + this.mob.getRandom().nextInt(ORBIT_DURATION_MAX_TICKS - ORBIT_DURATION_MIN_TICKS + 1);
  ...
  public void tick() {
    ...
    this.mob
        .getLookControl()
        .setLookAt(
            this.lightPos.getX() + 0.5, this.lightPos.getY() + 0.5, this.lightPos.getZ() + 0.5);
  ...
  private void pickAndMoveToNextHoverTarget() {
    ...
    this.mob
        .getNavigation()
        .moveTo(this.hoverTarget.x, this.hoverTarget.y, this.hoverTarget.z, this.speed);
  }
  ...
  private Vec3 pickHoverTarget(Level level, BlockPos lightPos) {
    ...
    for (int attempt = 0; attempt < 10; attempt++) {
      ...
      double radius =
          ORBIT_RADIUS_MIN
              + this.mob.getRandom().nextDouble() * (ORBIT_RADIUS_MAX - ORBIT_RADIUS_MIN);
  ...
  private Vec3 applySeparationBias(Level level, Vec3 target) {
    ...
    List<Mob> nearby =
        level.getEntitiesOfClass(
            Mob.class, box, e -> e != this.mob && e.getType() == this.mob.getType());
