...
public class FrostSacProjectile extends ThrowableItemProjectile {
  ...
  public FrostSacProjectile(EntityType<? extends FrostSacProjectile> type, Level level) {
    super(type, level);
  }
    ...
    if (!this.level().isClientSide) {
      this.level()
          .playSound(
              ...
              this.getY(),
              this.getZ(),
              ModSoundEvents.SAC_IMPACT.get(),
              SoundSource.PLAYERS,
              0.9F,
              0.8F + this.level().getRandom().nextFloat() * 0.4F);
      int durationSeconds = Math.max(1, HexaliaConfig.frostSacDuration());
      FrostCloud cloud =
          new FrostCloud(this.level(), this.getX(), this.getY(), this.getZ(), durationSeconds);
