...
public class ModGameEvents {
    ...
    private static final RegistryKey<DamageType> INDIRECT_MAGIC =
            RegistryKey.of(RegistryKeys.DAMAGE_TYPE, new Identifier("minecraft", "indirect_magic"));
    private static final RegistryKey<DamageType> WITHER =
            RegistryKey.of(RegistryKeys.DAMAGE_TYPE, new Identifier("minecraft", "wither"));
    private static final RegistryKey<DamageType> DRAGON_BREATH =
            RegistryKey.of(RegistryKeys.DAMAGE_TYPE, new Identifier("minecraft", "dragon_breath"));
    ...
    private static void registerBrambleguardDamageReduction() {
        ServerLivingEntityEvents.ALLOW_DAMAGE.register((entity, source, amount) -> {
            var instance = entity.getStatusEffect(ModMobEffects.BRAMBLEGUARD);
            if (instance == null) {
                return true;
            }
    ...
    private static void registerSiphonBlockBreak() {
        PlayerBlockBreakEvents.BEFORE.register((world, player, pos, state, blockEntity) -> {
            ...
            var instance = player.getStatusEffect(ModMobEffects.SIPHON);
            if (instance == null) {
                return true;
            }
    ...
    public static void handleHollowSilenceDarkness(PlayerEntity player) {
        ...
        if (player.getStatusEffect(ModMobEffects.HOLLOW_SILENCE) == null) {
            return;
        }
    ...
    public static boolean shouldGhostveilPreventTarget(LivingEntity mob, LivingEntity proposedTarget) {
        if (!(proposedTarget instanceof PlayerEntity player)) {
            return false;
        }
