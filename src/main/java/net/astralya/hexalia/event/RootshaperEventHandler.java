...
public class RootshaperEventHandler {
        ...
        PlayerBlockBreakEvents.BEFORE.register((world, player, pos, state, blockEntity) -> {
            ...
            try {
                for (BlockPos adjacent : get3x3Positions(pos, face)) {
                    ...
                    if (adjacentState.isAir() || adjacentState.getHardness(world, adjacent) < 0.0F) {
                        continue;
                    }
                    ...
                    if (broke && !serverPlayer.isCreative()) {
                        stack.damage(1, serverPlayer, entity -> entity.sendEquipmentBreakStatus(EquipmentSlot.MAINHAND));
                        if (stack.isEmpty()) {
                            break;
                        }
                    }
    ...
    public static void onLeftClickBlock(ServerPlayerEntity player, BlockPos pos, World world) {
        ...
        if (!stack.isOf(ModItems.ROOTSHAPER)) {
            return;
        }
