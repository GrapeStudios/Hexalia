...
public class SmallCauldronBlockEntityRenderer implements BlockEntityRenderer<SmallCauldronBlockEntity> {
    ...
    private void renderLiquid(SmallCauldronBlockEntity be, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int packedLight) {
        ...
        consumer.vertex(mat, LIQUID_MIN_X, height, LIQUID_MIN_Z).color(r, g, b, a).texture(u0, v0).overlay(overlay).light(packedLight).normal(0.0F, 1.0F, 0.0F).next();
        consumer.vertex(mat, LIQUID_MIN_X, height, LIQUID_MAX_Z).color(r, g, b, a).texture(u0, v1).overlay(overlay).light(packedLight).normal(0.0F, 1.0F, 0.0F).next();
        consumer.vertex(mat, LIQUID_MAX_X, height, LIQUID_MAX_Z).color(r, g, b, a).texture(u1, v1).overlay(overlay).light(packedLight).normal(0.0F, 1.0F, 0.0F).next();
        consumer.vertex(mat, LIQUID_MAX_X, height, LIQUID_MIN_Z).color(r, g, b, a).texture(u1, v0).overlay(overlay).light(packedLight).normal(0.0F, 1.0F, 0.0F).next();
    }
