...
public class CenserBlockEntityRenderer implements BlockEntityRenderer<CenserBlockEntity> {
    ...
    public void render(CenserBlockEntity censer, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        if (state.hasProperty(CenserBlock.LIT) && state.getValue(CenserBlock.LIT)) return;
        if (censer.getBurnTime() > 0) return;
    ...
    private void renderOne(Level level, CenserBlockEntity censer, ItemStack stack, PoseStack poseStack, MultiBufferSource buffer,
                           int packedLight, int packedOverlay, float x, float y, float z) {
        poseStack.pushPose();
