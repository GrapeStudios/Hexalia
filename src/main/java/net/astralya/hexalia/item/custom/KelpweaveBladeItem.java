...
public class KelpweaveBladeItem extends SwordItem {
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity livingEntity, int remainingUseTicks) {
        ...
        int usedTicks = getMaxUseTime(stack, livingEntity) - remainingUseTicks;
        if (usedTicks < MIN_CHARGE_TICKS) return;
        ...
        if (!world.isClient) {
            ...
            world.playSound(
                    null,
                    player.getBlockPos(),
                    SoundEvents.ITEM_TRIDENT_RIPTIDE_1.value(),
            ...
            if (!player.getAbilities().creativeMode) {
                stack.damage(DURABILITY_COST, player, player.getPreferredEquipmentSlot(stack));
            }
        }
    }
