...
public class EtherealGrazingEffect implements ICenserEffect {
    ...
    private static final EntityEffectParticleEffect GRAZE_PARTICLE =
            EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.2f, 0.75f, 0.2f);
    ...
    private static void triggerBreeding(ServerWorld world, Box area) {
        List<AnimalEntity> adults = world.getEntitiesByClass(AnimalEntity.class, area,
                a -> !a.isBaby() && a.getBreedingAge() == 0 && !a.isInLove());
    ...
    private static void spawnAmbientParticles(ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            world.spawnParticles(GRAZE_PARTICLE, x, y, z, 1, 0, 0, 0, 0);
        }
    }

    private static void spawnBreedingParticles(ServerWorld world, AnimalEntity animal) {
        world.spawnParticles(GRAZE_PARTICLE,
                animal.getX(), animal.getY() + animal.getHeight() * 0.5, animal.getZ(),
                10, 0.3, 0.3, 0.3, 0.05);
        world.spawnParticles(ParticleTypes.HEART,
                animal.getX(), animal.getY() + animal.getHeight(), animal.getZ(),
                3, 0.2, 0.1, 0.2, 0.0);
    }
}