...
public class BlightedBloomEffect implements ICenserEffect {
    ...
    private static void convertBlocks(ServerWorld world, BlockPos center, int radius) {
        ...
        for (BlockPos bp : BlockPos.iterate(min, max)) {
            ...
            if ((state.isOf(Blocks.GRASS_BLOCK) || state.isOf(Blocks.DIRT)) && isOnSpreadFrontier(world, bp, center)) {
        ...
        for (int i = 0; i < count; i++) {
            BlockPos target = pickBiasedNearest(candidates, world, center, radius);
            if (target == null) {
                break;
            }
    ...
    private static boolean isOnSpreadFrontier(ServerWorld world, BlockPos pos, BlockPos center) {
        if (pos.isWithinDistance(center, 2.0)) {
            return true;
        }

        for (BlockPos neighbor : neighbors(pos)) {
            if (world.getBlockState(neighbor).isOf(Blocks.MYCELIUM)) {
                return true;
            }
        }
    ...
    private static BlockPos pickBiasedNearest(List<BlockPos> candidates, ServerWorld world, BlockPos center, int radius) {
        if (candidates.isEmpty()) {
            return null;
        }
        ...
        double maxDist = Math.max((double) radius * radius, 1.0);

        for (int i = 0; i < candidates.size(); i++) {
            double distSq = candidates.get(i).getSquaredDistance(center);
            weights[i] = Math.exp(-SPREAD_BIAS * distSq / maxDist);
    ...
    private static void trySpawnMushrooms(ServerWorld world, BlockPos center, int radius) {
        ...
        BlockPos target = pickBiasedNearest(surfaces, world, center, radius);
        if (target == null) {
            return;
        }
        ...
        if (mushroom.canPlaceAt(world, target)) {
            ...
            world.playSound(null, target, SoundEvents.BLOCK_GRASS_PLACE, SoundCategory.BLOCKS, 0.4f, 0.8f + world.random.nextFloat() * 0.4f);
            spawnConversionParticles(world, target);
        }
    }

    private static void tryConvertCows(ServerWorld world, Box area) {
        List<CowEntity> cows = world.getEntitiesByClass(CowEntity.class, area, entity -> !(entity instanceof MooshroomEntity));

        for (CowEntity cow : cows) {
            if (world.random.nextFloat() > MOOSHROOM_CHANCE) {
                continue;
            }
