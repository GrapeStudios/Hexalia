...
public class MinersRespiteEffect implements ICenserEffect {
    ...
    private static void repairAnvilsInArea(ServerWorld world, BlockPos center, int radius) {
        ...
        for (BlockPos pos : BlockPos.iterate(min, max)) {
            ...
            if (repaired.contains(Properties.HORIZONTAL_FACING)) {
                repaired = repaired.with(Properties.HORIZONTAL_FACING, facing);
            }

            world.setBlockState(pos, repaired, 3);
            world.playSound(null, pos, SoundEvents.BLOCK_ANVIL_USE, SoundCategory.BLOCKS, 0.35f, 1.2f);
