...
public class TidalPullEffect implements ICenserEffect {
    ...
    private static final EntityEffectParticleEffect SWIRL_PARTICLE =
            EntityEffectParticleEffect.create(ParticleTypes.ENTITY_EFFECT, 0.0f, 0.55f, 1.0f);
    ...
    public void onTick(ServerWorld world, BlockPos pos) {
        ...
        entities.addAll(world.getEntitiesByClass(AnimalEntity.class, area, e -> true));
        entities.addAll(world.getEntitiesByClass(HostileEntity.class, area, e -> true));
        entities.addAll(world.getEntitiesByClass(ItemEntity.class, area, e -> true));
        for (Entity entity : entities) {
            Vec3d entityPos = entity.getPos();
            double distSq = entityPos.squaredDistanceTo(target);
            if (distSq <= PULL_TARGET_RADIUS * PULL_TARGET_RADIUS) continue;
    ...
    private static void spawnAmbientParticles(ServerWorld world, BlockPos pos, int radius) {
        for (int i = 0; i < 2; i++) {
            ...
            world.spawnParticles(SWIRL_PARTICLE, x, y, z, 1, 0, 0, 0, 0);
        }
    }
