...
public class TidalPullEffect implements ICenserEffect {
    ...
    private static final ColorParticleOption SWIRL_PARTICLE =
            ColorParticleOption.create(ParticleTypes.ENTITY_EFFECT, 0.0f, 0.55f, 1.0f);
    ...
    private static void spawnAmbientParticles(ServerLevel level, BlockPos pos, int radius) {
        for (int i = 0; i < 2; i++) {
            ...
            level.sendParticles(SWIRL_PARTICLE, x, y, z, 1, 0, 0, 0, 0);
        }
    }
    ...
    private static void spawnCurrentParticles(ServerLevel level, BlockPos pos, int radius) {
        ...
        double startY = pos.getY() + level.random.nextDouble() * 1.5;
