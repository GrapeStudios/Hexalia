...
public class TidesMemoryEffect implements ICenserEffect {
    ...
    private static BlockPos findWaterSurface(ServerWorld world, BlockPos center) {
        ...
        for (BlockPos pos : BlockPos.iterate(min, max)) {
            if (world.getBlockState(pos).isOf(Blocks.WATER) && world.getBlockState(pos.up()).isAir()) {
                surfaces.add(pos.toImmutable());
            }
        }
    ...
    private static void spawnLoot(ServerWorld world, BlockPos surface) {
        ...
        itemEntity.setVelocity(0.0, 0.2 + world.random.nextDouble() * 0.1, 0.0);
    ...
    private static void trySpawnGuardian(ServerWorld world, BlockPos center) {
        ...
        Vec3d spawnCenter = Vec3d.ofCenter(spawnPos);

        GuardianEntity guardian = new GuardianEntity(EntityType.GUARDIAN, world);
        guardian.refreshPositionAndAngles(spawnCenter.x, spawnCenter.y, spawnCenter.z, 0.0f, 0.0f);
        guardian.initialize(world, world.getLocalDifficulty(spawnPos), SpawnReason.TRIGGERED, null, null);
    ...
    private static void spawnAmbientParticles(ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            world.spawnParticles(ParticleTypes.DRIPPING_WATER, x, y, z, 1, 0.0, 0.0, 0.0, 0.0);
        }
    }
