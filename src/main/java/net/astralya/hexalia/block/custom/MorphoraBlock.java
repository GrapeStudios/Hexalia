    ...
    public boolean tryActivateWithMutavis(ServerWorld world, BlockPos morphoraPos, ItemStack mutavisStack, PlayerEntity player) {
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dz = -radius; dz <= radius; dz++) {
                ...
                SimpleInventory inventory = new SimpleInventory(inputStack.copy());
                Optional<MutationRecipe> match = world.getRecipeManager().getFirstMatch(ModRecipes.MUTATION_TYPE, inventory, world);

                if (match.isEmpty()) {
                    continue;
                }

                ItemStack result = match.get().craft(inventory, world.getRegistryManager());
