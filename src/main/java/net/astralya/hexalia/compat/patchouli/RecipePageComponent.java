...
public class RecipePageComponent implements ICustomComponent {
    ...
    public void render(DrawContext graphics, IComponentRenderContext context, float pticks, int mouseX, int mouseY) {
        ...
        graphics.drawText(
                ...
                x + (PAGE_WIDTH / 2) - (MinecraftClient.getInstance().textRenderer.getWidth(recipe.output.getName()) / 2),
                y,
                0xFF404040,
                false
        );
    ...
    private static Optional<RecipeView> loadRecipe(String recipeId, String recipeLayout) {
        ...
        return switch (recipeLayout) {
            case "small_cauldron" -> manager.get(id)
                    .filter(entry -> entry.value() instanceof SmallCauldronRecipe)
                    .map(entry -> {
                        SmallCauldronRecipe recipe = (SmallCauldronRecipe) entry.value();
                        return new RecipeView(List.copyOf(recipe.getIngredients()), recipe.getResult(null).copy());
                    });
            case "ritual_brazier" -> manager.get(id)
                    .filter(entry -> entry.value() instanceof RitualBrazierRecipe)
                    .map(entry -> {
                        RitualBrazierRecipe recipe = (RitualBrazierRecipe) entry.value();
                        return new RecipeView(List.of(recipe.inputItem()), recipe.output().copy());
                    });
            case "mutation" -> manager.get(id)
                    .filter(entry -> entry.value() instanceof MutationRecipe)
                    .map(entry -> {
                        MutationRecipe recipe = (MutationRecipe) entry.value();
                        return new RecipeView(List.of(recipe.inputItem()), recipe.output().copy());
                    });
            case "mortar_and_pestle" -> manager.get(id)
                    .filter(entry -> entry.value() instanceof MortarAndPestleRecipe)
                    .map(entry -> {
                        MortarAndPestleRecipe recipe = (MortarAndPestleRecipe) entry.value();
                        return new RecipeView(List.copyOf(recipe.getIngredients()), recipe.output().copy());
                    });
            case "ritual_table" -> manager.get(id)
                    .filter(entry -> entry.value() instanceof RitualTableRecipe)
                    .map(entry -> {
                        RitualTableRecipe recipe = (RitualTableRecipe) entry.value();
                        return new RecipeView(List.copyOf(recipe.getIngredients()), recipe.output().copy());
                    });
