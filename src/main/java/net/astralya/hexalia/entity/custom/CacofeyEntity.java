...
public class CacofeyEntity extends TameableEntity implements GeoEntity {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(6, new FollowOwnerGoal(this, 1.0D, 5.0F, 2.0F, false) {
            @Override
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(HELD_ITEM, ItemStack.EMPTY);
        this.dataTracker.startTracking(INSPECTING, false);
        this.dataTracker.startTracking(CACOFEY_MODE, (byte) CacofeyMode.FOLLOW.ordinal());
    }
        ...
        if (this.isTamed() && this.isOwner(player)) {
            if (stack.getItem() instanceof HexFocusItem) {
                if (!this.getWorld().isClient) {
                    ...
                    player.sendMessage(Text.translatable("message.hexalia.cacofey.attuned", this.getName()), true);
                }
    ...
    public void writeCustomDataToNbt(NbtCompound tag) {
        ...
        if (this.isHoldingItem()) {
            tag.put(TAG_HELD_ITEM, this.getHeldItem().writeNbt(new NbtCompound()));
        }
    ...
    public void readCustomDataFromNbt(NbtCompound tag) {
        ...
        if (tag.contains(TAG_MODE)) {
            setMode(CacofeyMode.values()[tag.getByte(TAG_MODE)]);
        }

        if (tag.contains(TAG_HELD_ITEM)) {
            this.setHeldItem(ItemStack.fromNbt(tag.getCompound(TAG_HELD_ITEM)));
        }
