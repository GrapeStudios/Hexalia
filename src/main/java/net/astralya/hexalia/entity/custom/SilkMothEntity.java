...
public class SilkMothEntity extends Animal implements GeoEntity {
    ...
    private SilkMothEntity pickEggLayer(Animal partner) {
        if (!(partner instanceof SilkMothEntity other)) return null;
        ...
        for (int dy = -down; dy <= up; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (!level.getBlockState(belowPos).is(ModBlocks.NESTING_BLOCK.get())) continue;
                    BlockPos placePos = belowPos.above();
                    if (!level.getBlockState(placePos).isAir()) continue;
                    validCount++;
                    if (chosen == null || this.random.nextInt(validCount) == 0) chosen = placePos;
                }
            }
        }
        return chosen;
    ...
    private BlockPos findEggLayPosAboveLeaves(ServerLevel level, BlockPos base, int radius, int down, int up) {
        ...
        for (int dy = -down; dy <= up; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    BlockPos belowPos = base.offset(dx, dy, dz);
                    if (!level.getBlockState(belowPos).is(BlockTags.LEAVES)) continue;
                    BlockPos placePos = belowPos.above();
                    if (!level.getBlockState(placePos).isAir()) continue;
                    validCount++;
                    if (chosen == null || this.random.nextInt(validCount) == 0) chosen = placePos;
    ...
    public boolean isEggReady() { return this.eggReady; }

    @Nullable
    public BlockPos getEggLayPos() { return this.eggLayPos; }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (bottleResult.consumesAction()) return bottleResult;
        return super.mobInteract(player, hand);
    }

    private InteractionResult tryBottle(Player player, ItemStack heldItem) {
        if (!heldItem.is(ModItems.RUSTIC_BOTTLE.get())) return InteractionResult.PASS;
        if (this.level().isClientSide) return InteractionResult.SUCCESS;
        ItemStack bottledMoth = new ItemStack(ModItems.BOTTLED_MOTH.get());
        CompoundTag entityTag = new CompoundTag();
        if (this.hasCustomName()) {
            entityTag.putString(net.astralya.hexalia.item.custom.BottleMothItem.MOTH_NAME, this.getCustomName().getString());
        }
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains(TAG_VARIANT)) this.setVariant(SilkMothVariant.byId(tag.getInt(TAG_VARIANT)));
        if (tag.contains(TAG_EGG_READY)) this.eggReady = tag.getBoolean(TAG_EGG_READY);
        if (tag.contains(TAG_EGG_COOLDOWN)) this.eggCooldownTicks = tag.getInt(TAG_EGG_COOLDOWN);
        if (tag.contains(TAG_EGG_POS)) this.eggLayPos = BlockPos.of(tag.getLong(TAG_EGG_POS));
        else this.eggLayPos = null;
        if (tag.contains(TAG_NAME)) this.setCustomName(Component.literal(tag.getString(TAG_NAME)));
    }
